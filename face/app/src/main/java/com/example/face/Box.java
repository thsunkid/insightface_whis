...
public class Box {
    ...
    public Rect transform2Rect(){
        ...
        int rong = rect.right - rect.left;
        int dai = rect.bottom - rect.top;
        if (rong< dai)
        {
            int delta = dai - rong;
            rect.right = rect.right +delta/2;
            rect.left = rect.left - delta/2;
        } else if (rong > dai)
        {
            int delta = rong - dai;
            rect.top = rect.top - delta/2;
            rect.bottom = rect.bottom + delta/2;
        }
        return  rect;
    }
    //面积
